...
//         typeOfExam: userData?.checkHistory[0]?.examType,
//         page,
//         limit: 20
//       };
...
//             Based on your rank: <span className="font-semibold text-gray-800">{userData?.checkHistory[0]?.rank || 'N/A'}</span> | 
//             Category: <span className="font-semibold text-gray-800">{userData?.checkHistory[0]?.category || 'N/A'}</span> | 
//             Gender: <span className="font-semibold text-gray-800">{userData?.checkHistory[0]?.gender || 'N/A'}</span>
//           </p>
//         </motion.div>
...
const PredictColleges = () => {
  ...
  const getUserProfileData = () => {
    // Check if userData has the fields directly or in checkHistory
    if (userData?.rank && userData?.category && userData?.gender && userData?.examType) {
      // Data is in root of userData
      return {
        rank: userData.rank,
  ...
  const fetchAllColleges = async (page = 1) => {
    ...
    if (!profileData) {
      ...
      const missingParams = [];
      if (!userData?.rank && !userData?.checkHistory?.[0]?.rank) missingParams.push('rank');
      if (!userData?.category && !userData?.checkHistory?.[0]?.category) missingParams.push('category');
      if (!userData?.gender && !userData?.checkHistory?.[0]?.gender) missingParams.push('gender');
      if (!userData?.examType && !userData?.checkHistory?.[0]?.examType) missingParams.push('examType');
    ...
    // Check for required parameters
    const requiredParams = ['rank', 'category', 'gender', 'examType'];
    const missingParams = requiredParams.filter(param => !profileData[param] && profileData[param] !== 0);
