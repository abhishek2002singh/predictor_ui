...
const PredictColleges = () => {
  ...
  // Initial data fetch - Load first 3 colleges
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const params = {
          ...
          typeOfExam: initialFormData.examType,
          page: 1,
          limit: 3
        };
        ...
        const response = await dispatch(fetchCutoffs(params)).unwrap();
  ...
  // Handle "Show All Colleges" button click - Opens the form
  const handleShowMore = () => {
    dispatch(setShowMoreForm(true));
  };

  // Handle "View All" button click - Fetches all colleges directly with pagination
  const handleViewAll = async () => {
    dispatch(setShowMoreForm(true));
    // try {
    //   setShowAllColleges(true);
    //   await fetchAllColleges(1); // Start with page 1
    // } catch (error) {
  ...
  const getUserProfileData = () => {
    // Priority: 1. location state, 2. userData from Redux, 3. checkHistory
                                    ...
                                    <div
                                      className={`h-full rounded-full ${college.probability === 'High Chance' ? 'bg-green-500' :
                                          college.probability === 'Medium Chance' ? 'bg-yellow-500' :
                                            college.probability === 'Low Chance' ? 'bg-orange-500' :
                                              'bg-red-500'
                                        }`}
          ...
          >
            ...
            >
              ...
              <p className="text-gray-600 mb-8 text-center">
                Please provide your details to view all {summaryStats.totalColleges} college predictions.
              </p>
              ...
              <form onSubmit={handleMoreFormSubmit} className="space-y-6">
                ...
                <p className="text-sm text-gray-500 text-center mt-4">
                  After submitting, you'll see all {summaryStats.totalColleges} colleges with pagination (20 per page)
                </p>
              </form>
            </motion.div>
          </motion.div>
